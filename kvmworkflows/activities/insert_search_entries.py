...
@activity.defn
async def upsert_search_entries(search_entries: Iterable[SearchEntry]) -> None:
    inputs = list(
        map(
    ...
    await graphql_client.upsert_entries(inputs)
...
async def test_insert_search_entries():
    ...
    await upsert_search_entries(search_entries)
